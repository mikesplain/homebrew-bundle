...
describe Bundle::TapInstaller do
  def do_install(options = {})
    Bundle::TapInstaller.install("phinze/cask", options)
  end

  def do_pinning(options = {})
    Bundle::TapInstaller.check_pinning("phinze/cask", options)
  end
  ...
  context "when tap is installed" do
    ...
    context "with pin true" do
      it "pins" do
        expect(Bundle).to receive(:system).with("brew", "tap-pin", "phinze/cask").and_return(true)
  ...
  context "when tap is pinned" do
    before do
      allow(Bundle::TapInstaller).to receive(:installed_taps).and_return(["phinze/cask"])
      allow(Bundle::TapInstaller).to receive(:pinned_installed_taps).and_return(["phinze/cask"])
      allow(ARGV).to receive(:verbose?).and_return(false)
    end

    context "with pin false" do
      it "unpins" do
        expect(Bundle).to receive(:system).with("brew", "tap-unpin", "phinze/cask").and_return(true)
        expect(do_install).to eql(:skipped)
      end
    end
  end
